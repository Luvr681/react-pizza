...
function Cart() {
  ...
  const messageForClearCart = 'Вы действительно хотите очистить корзину?';
  const messageForRemoveItem = 'Вы действительно хотите удалить?';

  const onClearCart = () => {
    if (window.confirm(messageForClearCart))
      dispatch(clearCart());
  }

  const onRemoveItem = id => {
    if (window.confirm(messageForRemoveItem))
